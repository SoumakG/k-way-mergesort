    ...
    public static void merge (int[] data, int low, int high, int k) {
        ...
        } else 
        {
            ...
            for (int i = 0, j = low; i < subarray.length; i++, j++) 
			{
             ...
             subarray[i] = new MergesortHeapNode(data[low + i*(high-low+1)/k], i ); // whichsubarray is marked by the iterative number of the subarray in data
				
               
			}
            BinaryHeap heap = BinaryHeap.buildHeap(subarray);
            int[] data1 = new int[high+1];
            int[] inSubarrayIndex = new int[k];
            for (int j = low; j <= high; j++) {
                try {
				MergesortHeapNode mh = (MergesortHeapNode) heap.deleteMin();
				 int whichsubarray = mh.getWhichSubarray();
				 inSubarrayIndex[whichsubarray]++;
                    data1[j] = mh.getKey();
					int nextElementInSubarrayIndex = low + whichsubarray*(high-low+1)/k + inSubarrayIndex[whichsubarray];
					if( nextElementInSubarrayIndex < (low + (whichsubarray+1)*(high-low+1)/k ))
					 heap.insert( new MergesortHeapNode(data[nextElementInSubarrayIndex], whichsubarray));
